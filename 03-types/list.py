...
# Generování seznamu čísel od 1 do 19999, které jsou dělitelné 200
hundreds = [num for num in range(1,2000) if not num % 200]
# Tento comprehension vytváří seznam čísel, která jsou násobky 200 mezi 1 a 20000 (např. 200, 400, 600...).
print(hundreds)

# Generování seznamu 50 náhodně vybraných velkých písmen z ASCII abecedy
ascii = [random.choice(string.ascii_uppercase) for _ in range(50)]
# Tento comprehension vybírá náhodné znaky z 'string.ascii_uppercase', které obsahuje všechna velká písmena.
print(ascii)

# Odebrání prvních a posledních tří prvků z seznamu 'hundreds'
hundreds[:3] = hundreds[-3:] = []
# Zde se odebírají prvky od začátku ([:3]) a od konce ([-3:]) seznamu 'hundreds'. Po této operaci budou první a poslední tři hodnoty odstraněny.
print(hundreds)

# Vytvoření seznamu unikátních písmen z 'ascii'
unique = [ _ for _ in ascii if not ascii.count(_) > 1]
# Tento comprehension vytváří seznam 'unique', který obsahuje pouze písmena z 'ascii', která se vyskytují jen jednou (jsou tedy unikátní).
print(unique)

# Spojení prvků seznamů 'hundreds' a 'ascii' do seznamu dvojic pomocí zip
combine = list(zip(hundreds, ascii[:len(hundreds)]))
# Funkce 'zip' spojuje prvky seznamů 'hundreds' a 'ascii' do dvojic (tuple) po délce kratšího seznamu.
# Vznikne seznam 'combine', který obsahuje dvojice hodnot z 'hundreds' a 'ascii' (např. [(200, 'A'), (400, 'B'), ...]).
print(combine)
...
print(f'\n*************************************\nCvičení 3\n*************************************')
# b) Výběr všech žen pomocí lambda a comprehension
women = [person[0] for person in persons if person[2] == 'žena']

# Výpis seznamu žen s podtržením podle délky jména
for woman in women:
    print(woman)
    print('-' * len(woman))


# c) Výběr osob s písmenem "i" ve jméně, řazení a formátování do CSV formátu

# Použití lambda pro výběr osob obsahujících "i"
ipeople = [(index + 1, person[0], person[1], person[2])
           for index, person in enumerate(sorted(persons, key=lambda x: x[1], reverse=True))
           if 'i' in person[0].lower()]

# Převod do CSV struktury
csv_output = "Index;Jméno;Věk;Pohlaví\n"
csv_output += "\n".join([f"{index};{name};{age};{gender}" for index, name, age, gender in ipeople])

# Výpis výsledného CSV
print("\nCSV struktura:")
print(csv_output)
